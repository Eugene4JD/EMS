...
public class TeacherListViewModel_detailsForExam
{
  private ObservableList<TeacherViewModel> list;
  private EMSModel model;

  public TeacherListViewModel_detailsForExam(EMSModel model)
  {
    this.model = model;
  ...
  public ObservableList<TeacherViewModel> update()
  {
    ...
    for (int i = 0; i < model.getTempExam().getTeachers().getNumberOfTeachers(); i++)
    {
      teachers.add(model.getTempExam().getTeachers().getTeacherByIndex(i));
    }
    list.clear();
