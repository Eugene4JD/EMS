...
public class StudentListViewModel_details
{
  private ObservableList<StudentViewModel> list;
  private EMSModel model;

  public StudentListViewModel_details(EMSModel model)
  {
    this.model = model;
  ...
  public ObservableList<StudentViewModel> update()
  {
    ...
    for (int i = 0; i < model.getTempClass().getNumberOfStudents(); i++)
    {
      students.add(model.getTempClass().getStudents().getStudentByIndex(i));
    }
    list.clear();
